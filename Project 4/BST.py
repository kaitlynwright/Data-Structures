...
class BinarySearchTree:
	...
	def bpv_helper(self, node):
		if node == None:
			return 0
		else:
			left = self.bpv_helper(node.left)
			right = self.bpv_helper(node.right)
		return max(left, right) + str(node.key).count('5')

	def best_path_value(self):
		return self.bpv_helper(self.root)
